...
It's important to understand that the shell has a specific standard functioning: a shell has the main function of starting new processes. On Unix-like systems, this is done by calling the _fork()_ syscall. What it does is that it duplicates the current running process, creating a new one that will run in parallel with the original one. This logic means that all the shell-created processes come from the same parent process, which is the shell process itself, loaded once the shell starts running.
